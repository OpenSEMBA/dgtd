...
{
	mfem::Segment s1(1, 4, 20), s2(4, 1, 30);
...
{
	mfem::Triangle t1(1, 2, 3);
	mfem::Triangle t2(3, 1, 2);
	mfem::Triangle t3(3, 2, 1);
...
TEST_F(GeometryTest, internal_boundary_orientation_2D)
{
	...
	//   V0 ----- V1 ------ V2
	//      f0         f4 
	auto m{ mfem::Mesh::MakeCartesian2D(2, 1, mfem::Element::Type::QUADRILATERAL) };
	mfem::Segment seg1(1, 4, 20);
	mfem::Segment seg2(4, 1, 30);

	int bdr1{ m.AddBdrElement(new mfem::Segment(seg1)) };
	int bdr2{ m.AddBdrElement(new mfem::Segment(seg2)) };
	m.FinalizeTopology();
	m.Finalize();
...
TEST_F(GeometryTest, orientation_from_gmsh_mesh_3D)
{
	...
	auto m { mfem::Mesh::LoadFromFileNoBdrFix((gmshMeshesFolder() + "test_normals_3D_all_bdr_normals_in.msh").c_str(), 1, 0, false) };
...
TEST_F(GeometryTest, orientation_from_gmsh_mesh_3D_bad_orientation)
{
	...
	auto m{ mfem::Mesh::LoadFromFileNoBdrFix((gmshMeshesFolder() + "test_normals_3D_no_reversal.msh").c_str(), 1, 0, false) };
