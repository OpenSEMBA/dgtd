	...
	opts_{ options }
{
	...
	for (int x = X; x <= Z; x++) {
		...
		allocateDenseInEigen(buildByMult(*buildInverseMassMatrix(H, model_, fes_), *buildFluxOperator(E, { y }, model_, fes_, opts_), fes_)->SpMat().ToDenseMatrix(), global_, { H,E }, { x,z }); // MFN
		allocateDenseInEigen(buildByMult(*buildInverseMassMatrix(H, model_, fes_), *buildFluxOperator(E, { z }, model_, fes_, opts_), fes_)->SpMat().ToDenseMatrix(), global_, { H,E }, { x,y }, -1.0);
		allocateDenseInEigen(buildByMult(*buildInverseMassMatrix(E, model_, fes_), *buildFluxOperator(H, { y }, model_, fes_, opts_), fes_)->SpMat().ToDenseMatrix(), global_, { E,H }, { x,z }, -1.0);
		allocateDenseInEigen(buildByMult(*buildInverseMassMatrix(E, model_, fes_), *buildFluxOperator(H, { z }, model_, fes_, opts_), fes_)->SpMat().ToDenseMatrix(), global_, { E,H }, { x,y });
