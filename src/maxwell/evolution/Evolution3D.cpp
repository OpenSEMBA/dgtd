	...
	opts_{ options }
{
	...
	for (auto f : { E, H }) { //IntBdrConds
		...
		for (auto d{ X }; d <= Z; d++) {
			for (auto d2{ X }; d2 <= Z; d2++) {
				for (auto f2 : { E, H }) {
					MFNB_[f][f2][d] = buildIBFIByMult(*buildInverseMassMatrix(f, model_, fes_), *buildFluxIBFIOperator(f2, { d }, model_, fes_, opts_), fes_);
...
void MaxwellEvolution3D::Mult(const Vector& in, Vector& out) const
{
	...
	for (int x = X; x <= Z; x++) {
		...
		if (opts_.fluxType == FluxType::Upwind) {
			...
			MFNN_[H][H][X][x]->AddMult(hOld[X], hNew[x], 0.5);
			MFNN_[H][H][Y][x]->AddMult(hOld[Y], hNew[x], 0.5);
			MFNN_[H][H][Z][x]->AddMult(hOld[Z], hNew[x], 0.5);
			MP_[H]			 ->AddMult(hOld[x], hNew[x],-0.5);
			
			MFNN_[E][E][Y][x]->AddMult(eOld[Y], eNew[x], 0.5);
			MFNN_[E][E][X][x]->AddMult(eOld[X], eNew[x], 0.5);
			MFNN_[E][E][Z][x]->AddMult(eOld[Z], eNew[x], 0.5);
			MP_[E]			 ->AddMult(eOld[x], eNew[x],-0.5);
		}
