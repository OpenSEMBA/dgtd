...
const FiniteElementOperator buildByMult(
...
FiniteElementOperator buildInverseMassMatrix(const FieldType& f, const Model& model, const FiniteElementSpace& fes)
{
...
FiniteElementOperator buildDerivativeOperator(const Direction& d, const FiniteElementSpace& fes)
{
	auto res = std::make_unique<BilinearForm>(&fes);
...
FiniteElementOperator buildFluxOperator(const FieldType& f, const Direction& d, bool usePenaltyCoefficients, Model& model, const FiniteElementSpace& fes, const MaxwellEvolOptions& opts)
{
	auto res = std::make_unique<BilinearForm>(&fes);
	if (d >= fes.GetMesh()->Dimension()) {
		res->Assemble();
