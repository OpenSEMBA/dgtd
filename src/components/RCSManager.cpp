...
PlaneWaveData RCSManager::buildPlaneWaveData(const json& json)
{
	...
	return PlaneWaveData(mean / speed_of_light, delay / speed_of_light);
}
		...
		if (dir_entry.path().generic_string().substr(dir_entry.path().generic_string().size() - 4) != "mesh") {
			...
			for (int f{ 0 }; f < frequencies.size(); f++) {
				...
				for (int i{ 0 }; i < dofs; i++) {
					res[f][i] += std::conj(std::complex<double>(gf[i] * cos(-2.0 * M_PI * (frequencies[f] / speed_of_light) * time), gf[i] * sin(-2.0 * M_PI * (frequencies[f] / speed_of_light) * time)));
				}
			}
		}
	}
...
RCSManager::RCSManager(const std::string& path, const std::string& json_path, const std::vector<double>& frequencies, const std::vector<SphericalAngles>& angle_vec)
{
	...
	for (int f{ 0 }; f < frequencies.size(); f++) {
		for (const auto& angpair : angle_vec) {
			...
			case 2:	
				...
				const_term = std::pow((2.0 * M_PI * (frequencies[f]/speed_of_light)), 2.0) / (8.0 * M_PI * eta_0 * normalization_term[f]);
	...
	for (const auto& angpair : angle_vec) {
		...
		for (const auto& f : frequencies) {
			myfile << angpair.first << " " << angpair.second << " " << f << " " << 10.0*log10(postdata_[angpair][f]/(speed_of_light/f)) << "\n";
		}
		myfile.close();
	}
